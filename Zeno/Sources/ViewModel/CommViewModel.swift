...
class CommViewModel: ObservableObject {
		...
		return allComm
			...
			.filter { allComm in
				if joinedComm.isEmpty {
					...
					return joinedComm.contains { return $0.id != allComm.id }
				}
			}
	}
	...
	private func fetchCurrentWaitMembers() async {
		guard let currentWaitMemberIDs = currentComm?.waitApprovalMemberIDs.map({ $0 }) else { return }
